...
class ProfileForm(forms.ModelForm):
    ...
    date_of_birth = forms.DateField(input_formats=['%Y-%m-%d', '%m/%d/%Y', ],
                                    widget=forms.DateInput(format='%Y-%m-%d'))
    short_bio = forms.CharField(widget=TinyMCE(
                                attrs={'rows': 10}), min_length=10)
...
class PasswordChangeCustomForm(forms.Form):
    ...
    def clean(self):
        ...
        if (self.profile.first_name.lower() in new_password.lower() or
